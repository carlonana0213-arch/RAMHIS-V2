...
const Analytics = () => {
  ...
  const exportPDF = async () => {
    try {
      ...
      pdf.text("RAMHIS Predictive Analytics Report", pageWidth / 2, 20, {
        align: "center",
      });
      ...
      pdf.text(`Location: ${selectedLocation}`, pageWidth / 2, 34, {
        align: "center",
      });

      pdf.text(`Mission Duration: ${missionDays} day(s)`, pageWidth / 2, 40, {
        align: "center",
      });
      ...
      autoTable(pdf, {
        ...
        head: [["Metric", "Value"]],
        body: [
          ["Forecasted Registration", analytics?.predictedPatients || 0],
          ["Confidence Tier", analytics?.confidence || "LOW"],
          [
            "Prediction Model",
      ...
      pdf.text("Department Volume Forecast", 14, pdf.lastAutoTable.finalY + 15);

      autoTable(pdf, {
        startY: pdf.lastAutoTable.finalY + 20,
        head: [["Department", "Patients"]],
        body: departmentRows.map(([dept, count]) => [
          dept,
          `${count} patients`,
        ]),
      });
      ...
      pdf.text("Top Anticipated Diagnoses", 14, pdf.lastAutoTable.finalY + 15);

      autoTable(pdf, {
        startY: pdf.lastAutoTable.finalY + 20,
        head: [["Diagnosis", "Count"]],
        body: (analytics?.topDiagnoses || []).map((item) => [
          item.diagnosis,
          item.count,
        ]),
      });
      ...
      pdf.text("Medicine Resource Forecast", 14, pdf.lastAutoTable.finalY + 15);

      autoTable(pdf, {
        startY: pdf.lastAutoTable.finalY + 20,
        head: [["Medicine", "Estimated Need", "Risk"]],
        body: (analytics?.medicineForecast || []).map((med) => [
          med.medicine,
          med.estimatedNeed,
          med.risk,
        ]),
      ...
      autoTable(pdf, {
        startY: pdf.lastAutoTable.finalY + 20,
        head: [["Recommendation"]],
        body: operationalInsights.map((item) => [item.text]),
      });
      ...
      pdf.text("Generated by RAMHIS Predictive Analytics", pageWidth / 2, 285, {
        align: "center",
      });
